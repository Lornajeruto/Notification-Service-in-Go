...
func SendEmail(c *gin.Context) {
	...
	d := mail.NewDialer(config.Email.SMTPHost, config.Email.SMTPPort, config.Email.Username, config.Email.Password)
...
func SendPushNotification(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to initialized Firebase app"})
		return
	}
